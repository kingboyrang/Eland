...
- (void)drawRect:(CGRect)rect {
    ...
    [[UIColor colorFromHexRGB:@"3db5c0"] setStroke];
...
- (void)selectedViewcontrollerIndex:(NSInteger)index{
    self.animatingToTab = YES;
...
- (void)reloadData {
    ...
    _tabsView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0, 0.0, [[UIScreen mainScreen] bounds].size.width, self.tabHeight)];
    //_tabsView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    //_tabsView.backgroundColor = self.tabsViewBackgroundColor;
    _tabsView.backgroundColor=[UIColor colorFromHexRGB:@"3db5c0"];
    ...
    CGFloat contentSizeWidth = 0,leftX=0;
    for (int i = 0; i < _tabCount; i++) {
        ...
        CGRect frame = tabView.frame;
        if (i==0) {
            leftX=([[UIScreen mainScreen] bounds].size.width-w*4)/2;
        }
        frame.origin.x = leftX;
        frame.origin.y=(self.tabHeight-h)/2;
    ...
    _tabsView.contentSize = CGSizeMake([[UIScreen mainScreen] bounds].size.width, self.tabHeight);
    
    _menuBar=[[MenuBar alloc] initWithFrame:_tabsView.frame];
    [_tabsView addSubview:_menuBar];
